...
fn main() {
    ...
    let libmlx_src = find_libmlx(&build_dir).unwrap_or_else(|| {
        panic!("could not find libmlx.a under {}", build_dir.display())
    });
    ...
    // Phase 2 will plug optional bindgen here under
    // cfg(feature = "buildtime_bindgen"). For now we ship pre-committed
    // bindings; bindgen runs only via `cargo run -p xtask -- regen-bindings`.
}
