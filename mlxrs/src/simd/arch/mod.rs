...
//! primitives.
//!
//! This whole module is compiled only under `all(feature = "simd",
//! target_arch = "aarch64")` — the single backend that exists today is
//! NEON. Backends supply f64 outputs **bit-identical** to
...
//!
//! There is no x86_64 backend today — `x86_64` builds (and `aarch64`
//! builds with the `simd` feature off) route through
