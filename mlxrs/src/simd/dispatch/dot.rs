...
//! targets, the `simd` feature off, or `--cfg mlxrs_force_scalar`).
...
///
/// Routes to NEON on `aarch64` (when the `simd` feature is on and the
/// CPU reports NEON), else to [`crate::simd::scalar::dot`]. Callers
/// needing byte-identical scalar output across every build
/// configuration call [`crate::simd::scalar::dot`] directly.
///
/// # Panics
///
...
/// (when the `simd` feature is on and the CPU reports NEON), else to
